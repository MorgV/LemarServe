...
class DeviceController {
  ...
  async getAll(req, res) {
    ...
    // text = 1
    if (text) {
      let zapros = await db.query(
        "SELECT devices.* FROM devices,device_tables WHERE( devices.name LIKE '%" +
          text +
          "%' or device_tables.serialnumber  LIKE '%" +
          text +
          "%') and  device_tables.id = devices.id "
      );
      let otvet = {
        rows: zapros[0],
